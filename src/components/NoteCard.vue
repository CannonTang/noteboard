<template>
  <article class="note-card" :class="`note-${note.color}`">
    ...
    <div v-if="note.images.length" class="card-images" :class="{ single: note.images.length === 1 }">
      <button v-for="image in note.images" :key="image.id" type="button" @click="emit('preview', image.src, image.name)"><img :src="image.src" :alt="image.name" /></button>
    </div>
...
const emit = defineEmits<{ edit: [note: Note]; delete: [note: Note]; preview: [src: string, name: string] }>()
