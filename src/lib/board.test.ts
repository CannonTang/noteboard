  ...
  it("keeps a saved order and adds new categories deterministically", () => {
    expect(
      normalizeCategoryOrder(
        ["Planning"],
        [...notes, { ...notes[0], id: "four", category: "Research" }],
      ),
    ).toEqual(["Planning", "Design", "Research"]);
  });

  it("keeps uncategorized notes last", () => {
    expect(
      groupNotes(notes, ["Planning", "Design"], "updated").map(
        (group) => group.category,
      ),
    ).toEqual(["Planning", "Design", "Uncategorized"]);
  });
