...
export function categoriesFor(notes: Note[]): string[] {
  return [
    ...new Set(notes.map((note) => normalizeCategory(note.category))),
...
export function sortNotes(notes: Note[], mode: SortMode): Note[] {
  return [...notes].sort((left, right) => {
    ...
    const leftValue = new Date(
      mode === "updated" ? left.updatedAt : left.createdAt,
    ).getTime();
    const rightValue = new Date(
      mode === "updated" ? right.updatedAt : right.createdAt,
    ).getTime();
...
export function groupNotes(
  ...
  return uncategorized
    ? [
        ...groups,
        { category: UNCATEGORIZED, notes: sortNotes(uncategorized, mode) },
      ]
    : groups;
}
